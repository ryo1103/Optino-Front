...
export const simplifyStr = (
  str: string | null,
  config?: SimplifyStrConfig
): string => {
  const defaultConfig = {
    frontBeginIndex: 0,
    frontLength: 6,
  ...
  const { frontBeginIndex, frontLength, connectSymbol, endingLength } = {
    ...defaultConfig,
    ...config,
  };
  return str
    ? `${str.substring(
        frontBeginIndex,
        frontBeginIndex + frontLength
      )}${connectSymbol}${str.substring(str.length - endingLength)}`
    : "-";
};
...
export const thousandthConvert = (
  n: number | string,
  config?: thousandthConvertConfig
): string => {
...
export const getLiquidityPoolContract = (
  ...
  return getContract(LPShares, lpAbi, library, account);
};
...
// @ts-ignore
export const toDecimals = (value: string, decimals = 18) => {
  console.log(value);
...
export const checkExpires = (allList: any) => {
  const len = allList.length;
  let res: any[] = [];
  for (let i = 0; i < len; i++) {
    const resExpire = ethers.BigNumber.from(allList[i][0]).toNumber();
    if (resExpire > dayjs().unix()) {
      res = allList[i];
      return res;
    }
  }
